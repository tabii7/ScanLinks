        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-600 bg-gray-800 text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent placeholder-gray-400"
            />
          </div>
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2 border border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-800 text-white">
            {['all','completed','generating','failed'].map(v => (<option key={v} value={v}>{v === 'all' ? 'All Status' : v}</option>))}
          </select>
          <select value={filterRegion} onChange={(e) => setFilterRegion(e.target.value)} className="px-4 py-2 border border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-800 text-white">
            {['all','US','UK','CA','AU'].map(v => (<option key={v} value={v}>{v === 'all' ? 'All Regions' : v}</option>))}
          </select>
          <select value={filterClient} onChange={(e) => setFilterClient(e.target.value)} className="px-4 py-2 border border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-800 text-white">
            {clientOptions.map(opt => (
              <option key={String(opt.value)} value={opt.value}>{opt.label}</option>
            ))}
          </select>
          <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="px-4 py-2 border border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-gray-800 text-white">
          ...
          <motion.div key={report.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-700 ar-surface">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-semibold text-white">{report.clientName}</h3>
              ...
              <div className={`px-2 py-1 rounded-full text-xs font-medium ${report.status === 'completed' ? 'bg-green-100 text-green-800' : report.status === 'generating' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                {report.status}
              </div>
            </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-white">{new Date(report.createdAt).toLocaleDateString()}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-400">Total Links</span>
                <span className="text-sm font-medium text-white">{report.summary?.totalLinks || 0}</span>
              </div>
              <div className="flex items-center justify-between">
            ...
            <div className="flex flex-nowrap items-center gap-2 mt-2">
              <button onClick={() => downloadReport(report.id, 'pdf')} title="Download PDF" className="h-9 w-9 rounded-full bg-red-600 hover:bg-red-700 flex items-center justify-center text-white transition-all duration-150 shadow focus:outline-none">
                <FileDown className="h-5 w-5" />
              </button>
              <button onClick={() => downloadReport(report.id, 'excel')} title="Download Excel" className="h-9 w-9 rounded-full bg-green-600 hover:bg-green-700 flex items-center justify-center text-white transition-all duration-150 shadow focus:outline-none">
                <FileSpreadsheet className="h-5 w-5" />
              </button>
              <button onClick={() => createChildScan(report)} title="Add Child Scan" className="h-9 w-9 rounded-full bg-indigo-600 hover:bg-indigo-700 flex items-center justify-center text-white transition-all duration-150 shadow focus:outline-none">
              ...
              <button
                ...
                className={`h-9 w-9 rounded-full flex items-center justify-center shadow focus:outline-none transition-all duration-150 ${report.clientStatus === 'sent' ? 'bg-gray-400 text-gray-100 cursor-default opacity-60' : 'bg-teal-600 hover:bg-teal-700 text-white'}`}
              >
                <Send className="h-5 w-5" />
              </button>
              <button onClick={() => window.open(`/admin/reports/${report.parentScanId}`, '_blank')} title="View Details" className="h-9 w-9 rounded-full bg-blue-600 hover:bg-purple-700 flex items-center justify-center text-white transition-all duration-150 shadow focus:outline-none">
