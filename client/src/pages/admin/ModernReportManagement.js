...
const ScheduleCountdown = ({ report, onRescan }) => {
      ...
      <span className="text-xs font-medium px-2 py-1 rounded-full border border-yellow-300 bg-yellow-900 text-yellow-200">
        ...
        {onRescan && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onRescan(report.parentScanId);
            }}
            className="text-xs font-medium px-3 py-1 rounded-full bg-blue-600 hover:bg-blue-700 text-white transition-colors"
          >
            Rescan
          </button>
        )}
...
Generated by ACE REPUTATIONS ORM Platform
  ...
  const handleRescan = async (scanId) => {
    try {
      if (!scanId) {
        toast.error('No scan ID found');
        return;
      }
      // Trigger a new scan using the same configuration as the parent
      const scanResponse = await api.get(`/scans/${scanId}`);
      const parentScan = scanResponse.data;
      
      if (!parentScan) {
        toast.error('Parent scan not found');
        return;
      }

      // Create a new child scan (which is essentially a rescan)
      await api.post(`/scans/${scanId}/create-child`);
      toast.success('Rescan triggered successfully');
      fetchReports();
    } catch (error) {
          ...
          <motion.div key={report.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-700 ar-surface">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between">
                ...
                {report.parentScanId ? (
                  <ScheduleCountdown report={report} onRescan={(scanId) => handleRescan(scanId)} />
                ) : (
                  <span className="text-xs font-medium px-2 py-1 rounded-full border border-gray-600 bg-gray-800 text-gray-300">
                    Not scheduled
                  </span>
            ...
            {/* Action buttons with text labels - compact single line */}
            <div className="flex items-center gap-0.5 mt-2">
              <button onClick={() => downloadReport(report.id, 'pdf')} className="flex items-center gap-0.5 px-1.5 py-1 rounded-md bg-red-600 hover:bg-red-700 text-white transition-all duration-150 shadow focus:outline-none text-xs whitespace-nowrap">
                <FileDown className="h-2.5 w-2.5" />
                <span>PDF</span>
              </button>
              <button onClick={() => downloadReport(report.id, 'excel')} className="flex items-center gap-0.5 px-1.5 py-1 rounded-md bg-green-600 hover:bg-green-700 text-white transition-all duration-150 shadow focus:outline-none text-xs whitespace-nowrap">
                <FileSpreadsheet className="h-2.5 w-2.5" />
                <span>Excel</span>
              </button>
              <button onClick={() => handleRescan(report.parentScanId)} className="flex items-center gap-0.5 px-1.5 py-1 rounded-md bg-indigo-600 hover:bg-indigo-700 text-white transition-all duration-150 shadow focus:outline-none text-xs whitespace-nowrap">
              ...
              <button
                ...
                className={`flex items-center gap-0.5 px-1.5 py-1 rounded-md shadow focus:outline-none transition-all duration-150 text-xs whitespace-nowrap ${report.clientStatus === 'sent' ? 'bg-gray-400 text-gray-100 cursor-default opacity-60' : 'bg-teal-600 hover:bg-teal-700 text-white'}`}
              >
                <Send className="h-2.5 w-2.5" />
                <span>Send</span>
              </button>
              <button onClick={() => window.open(`/admin/reports/${report.parentScanId}`, '_blank')} className="flex items-center gap-0.5 px-1.5 py-1 rounded-md bg-blue-600 hover:bg-purple-700 text-white transition-all duration-150 shadow focus:outline-none text-xs whitespace-nowrap">
